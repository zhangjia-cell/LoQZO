...
TASK=${TASK:-SST2}
RUN_PY=${RUN_PY:-Code/train/run.py}
LOG_HOME=${LOG_HOME:-./outputs}
TAG=${TAG:-}
LOG_PREFIX=${LOG_PREFIX:-quzo}
...
TRAINER_MODULE=${TRAINER_MODULE:-trainer_new}
MODE=${MODE:-ft}
TYPE=${TYPE:-ft}
OPTIM=${OPTIM:-sgd}
...
LOQZO_ENABLE=${LOQZO_ENABLE:-False}
...
case "${TRAINER,,}" in quzo) TRAINER_ECHO="zo_lowbit" ;; quzo_ft) TRAINER_ECHO="zo_lowbit_ft" ;; mezo) TRAINER_ECHO="zo" ;; fo) TRAINER_ECHO="regular" ;; esac
...
#
# 1) QuZO 主实验（全参数）
# GPU_ID=0 METHOD=quzo MODE=ft TASK=SST2 MODEL=facebook/opt-1.3b TAG=quzo_sst2 TRAIN=1000 DEV=500 EVAL=1000 STEPS=5000 SAVE_STEPS=500 LOGGING_STEPS=20 MAX_LENGTH=512 PREFER_LOCAL_MODEL=False LOG_HOME=./outputs bash Code/script/quzo.sh --wbit 4 --abit 8
#
# 2) 对照组 QuZO（MultiRC）
# GPU_ID=0 METHOD=quzo MODE=ft TASK=MultiRC MODEL=facebook/opt-1.3b TAG=quzo_multirc TRAIN=1000 DEV=500 EVAL=1000 STEPS=5000 SAVE_STEPS=500 LOGGING_STEPS=20 MAX_LENGTH=512 PREFER_LOCAL_MODEL=False LOG_HOME=./outputs bash Code/script/quzo.sh --wbit 4 --abit 8
#
# 3) 对照组 MeZO
# GPU_ID=0 METHOD=mezo MODE=ft TASK=MultiRC MODEL=facebook/opt-1.3b TAG=mezo_multirc TRAIN=1000 DEV=500 EVAL=1000 STEPS=5000 SAVE_STEPS=500 LOGGING_STEPS=20 MAX_LENGTH=512 PREFER_LOCAL_MODEL=False LOG_HOME=./outputs bash Code/script/quzo.sh
#
# 4) 对照组 FO
# GPU_ID=0 METHOD=fo MODE=ft TASK=SST2 MODEL=facebook/opt-1.3b TAG=fo_sst2 TRAIN=1000 DEV=500 EVAL=1000 STEPS=5000 SAVE_STEPS=500 LOGGING_STEPS=20 MAX_LENGTH=512 LOAD_BFLOAT16=True GRADIENT_CHECKPOINTING=True PREFER_LOCAL_MODEL=False LOG_HOME=./outputs bash Code/script/quzo.sh
#
# 5) LoRA（全精度底座）
# GPU_ID=0 METHOD=quzo MODE=lora TASK=MultiRC MODEL=facebook/opt-1.3b TAG=quzo_lora TRAIN=1000 DEV=500 EVAL=1000 STEPS=5000 SAVE_STEPS=500 LOGGING_STEPS=20 MAX_LENGTH=512 FORCE_DISABLE_BNB=True PREFER_LOCAL_MODEL=False LOG_HOME=./outputs bash Code/script/quzo.sh
#
# 6) LoRA + 8bit/4bit 底座（需 bitsandbytes 环境正常）
# GPU_ID=0 METHOD=quzo MODE=lora WBIT=8 TASK=SQuAD MODEL=meta-llama/Meta-Llama-3-8B TAG=quzo_lora_int8 TRAIN=1000 DEV=500 EVAL=1000 STEPS=3000 SAVE_STEPS=500 LOGGING_STEPS=20 MAX_LENGTH=512 PREFER_LOCAL_MODEL=False LOG_HOME=./outputs bash Code/script/quzo.sh
#
# 7) qft-int
# GPU_ID=0 METHOD=fo MODE=qft WBIT=4 ABIT=8 QMODE=int TASK=BoolQ MODEL=facebook/opt-1.3b TAG=qft_int TRAIN=1000 DEV=500 EVAL=1000 STEPS=1000 SAVE_STEPS=500 LOGGING_STEPS=20 MAX_LENGTH=256 PREFER_LOCAL_MODEL=False LOG_HOME=./outputs bash Code/script/quzo.sh
#
# 8) qft-float
# GPU_ID=0 METHOD=fo MODE=qft WBIT=4 ABIT=8 QMODE=float TASK=BoolQ MODEL=facebook/opt-1.3b TAG=qft_float TRAIN=1000 DEV=500 EVAL=1000 STEPS=1000 SAVE_STEPS=500 LOGGING_STEPS=20 MAX_LENGTH=256 PREFER_LOCAL_MODEL=False LOG_HOME=./outputs bash Code/script/quzo.sh
#
# 9) 多卡 model_parallel（适合 QuZO 大模型）
# GPU_IDS=0,1,2,3 LAUNCH_MODE=model_parallel METHOD=quzo MODE=ft TASK=MultiRC MODEL=facebook/opt-13b TAG=quzo_mp TRAIN=256 DEV=64 EVAL=128 BS=1 STEPS=500 SAVE_STEPS=500 LOGGING_STEPS=20 MAX_LENGTH=256 PREFER_LOCAL_MODEL=False LOG_HOME=./outputs bash Code/script/quzo.sh --wbit 4 --abit 8
#
# 10) 多卡 DDP（适合 FO / LoRA）
# GPU_IDS=0,1,2,3 LAUNCH_MODE=ddp METHOD=fo MODE=ft TASK=SST2 MODEL=facebook/opt-1.3b TAG=fo_ddp TRAIN=1000 DEV=500 EVAL=1000 BS=2 GRAD_ACC=8 STEPS=5000 SAVE_STEPS=500 LOGGING_STEPS=20 MAX_LENGTH=512 LOAD_BFLOAT16=True GRADIENT_CHECKPOINTING=True PREFER_LOCAL_MODEL=False LOG_HOME=./outputs bash Code/script/quzo.sh
#
# 11) 断点续训
# GPU_ID=0 METHOD=quzo MODE=ft TASK=SST2 MODEL=facebook/opt-1.3b TAG=quzo_resume TRAIN=1000 DEV=500 EVAL=1000 STEPS=5000 SAVE_STEPS=500 LOGGING_STEPS=20 MAX_LENGTH=512 RESUME_FROM_CHECKPOINT=./outputs/<run_name>/checkpoint-1500 PREFER_LOCAL_MODEL=False LOG_HOME=./outputs bash Code/script/quzo.sh --wbit 4 --abit 8
